...
export default function TagManager({ initialTags }: { initialTags: TagRow[] }) {
  ...
  // Hydrate from API so new tags persist across reloads during dev
  useEffect(() => {
    let mounted = true;
    apiList().then((rows) => {
      if (!mounted) return;
  ...
  function onDelete(name: string) {
    ...
    startTransition(() => {
      // optimistic
      const prev = tags;
      setTags((p) => p.filter((t) => t.name !== name));
                  ...
                  <div className='mt-2 space-y-3'>
                    ...
                    <div className='grid gap-2'>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setDraft((d) =>
                            d ? { ...d, count: Math.max(0, Number(e.target.value || 0)) } : d,
                          )
                        }
                      />
                    </div>
            ...
            {filtered.map((t) => (
              <div key={t.name} className='group rounded-xl border border-zinc-800 bg-zinc-950 p-4'>
                ...
                <div className='flex items-center gap-2'>
                  <Button
                    onClick={() => onEdit(t)}
                    className='h-9 flex-1 bg-zinc-100 text-zinc-900 hover:bg-white focus-visible:ring-2 focus-visible:ring-zinc-500'>
                  ...
                  <Button
                    ...
                    variant='outline'
                    className='h-9 flex-1 border-zinc-600 bg-zinc-900 text-zinc-200 hover:bg-zinc-800 focus-visible:ring-2 focus-visible:ring-zinc-500'>
